...
Here's a simple example that demonstrates how to create an agent with enforced tool execution order:
...
Building agents that work reliably across multiple LLMs is difficult.

Most current agents are tightly tuned to specific models. Their prompts are often rigid, verbose, or overly structured - causing models to misinterpret instructions, skip tool use, or hallucinate facts.

To address this, many teams resort to structured workflows or graphs. These improve reliability but come at the cost of flexibility and ease of development.
...
When multiple requirements generate conflicting rules:
