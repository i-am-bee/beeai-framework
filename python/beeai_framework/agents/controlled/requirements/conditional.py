...
TInput = TypeVar("TInput", bound=AbilityAgentRunState)
...
class ConditionalRequirement(Generic[TInput], Requirement[TInput]):
    def __init__(
        self,
        source: str,  # TODO: allow user to pass tools or not?
        *,
        name: str | None = None,
        force_at_step: int | None = None,
        only_before: list[str] | str | None = None,
        only_after: list[str] | str | None = None,
        force_after: list[str] | str | None = None,
        ...
        self.source = source
        self.name = name or f"Condition{source.capitalize()}"

        if priority is not None:
            self.priority = priority

        def extract_name(target: list[str] | str | list[AnyTool] | AnyTool | None) -> set[str]:
            return set[str](
    ...
    def init(self, *, tools: list[AnyTool], ctx: RunContext) -> None:
        existing_names = {a.name for a in tools}
    ...
    async def run(self, input: TInput, context: RunContext) -> list[RequirementResult]:
        ...
        invocations = sum(1 if step.tool and step.tool.name == self.source else 0 for step in steps)
        ...
        def resolve(allowed: bool) -> list[RequirementResult]:
            if not allowed and self._force_at_step == len(steps):
                raise ValueError(
                    f"Tool '{self.source}' cannot be executed at step {self._force_at_step} "
        ...
        if not self._can_be_used_in_row and self.source == last_tool_name:
