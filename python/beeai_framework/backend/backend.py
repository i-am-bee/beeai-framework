...
class Backend:
    ...
    def from_name(chat: str | ProviderName, embedding: str | ProviderName | None = None) -> "Backend":
        return Backend(ChatModel.from_name(chat), EmbeddingModel.from_name(embedding or "dummy"))

    @staticmethod
    def from_provider(name: str | ProviderName) -> "Backend":
        return Backend.from_name(name, name)

    async def clone(self) -> "Backend":
        return Backend(await self.chat.clone(), self.embedding.clone())
