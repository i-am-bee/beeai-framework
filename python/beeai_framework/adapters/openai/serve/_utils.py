...
def beeai_message_to_openai_message(message: AnyMessage) -> list[openai_api.ChatMessage]:
    messages: list[openai_api.ChatMessage] = []
    match message:
        case UserMessage():
            messages.append(openai_api.ChatMessage(role="user", content=message.text))
        case AssistantMessage():
            for text_msg in message.get_text_messages():
                messages.append(openai_api.ChatMessage(role="assistant", content=text_msg.text))
            if message.get_tool_calls():
                messages.append(
                    openai_api.ChatMessage(
                        ...
                        content=None,
                        tool_calls=[
                            openai_api.ChatToolCall(
                                id=t.id,
                                function=openai_api.ChatToolFunctionDefinition(
        ...
        case ToolMessage():
            for msg in message.get_tool_results():
                messages.append(openai_api.ChatMessage(role="tool", content=msg.result, tool_call_id=msg.tool_call_id))
        case SystemMessage():
            messages.append(openai_api.ChatMessage(role="system", content=message.text))
        case _:
            logger.warning(f"Message {message.to_plain()} could not be converted to OpenAI message. Skipping.")
