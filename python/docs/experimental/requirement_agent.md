...
agent = RequirementAgent(
  ...
  instructions="Recommend activities in a given destination based on the current time and weather.",
  requirements=[
...
- **Everything is a tool:** From data retrieval and web search to reasoning or even providing a final answer.
...
- **Tool execution is influenced by user-defined requirements:**
  - Always use tool `A` after tool `B`.
  - Before finishing, you must use tool `C`.
  - Tool `D` must be run exactly twice.
...
**Rules** convey the following information, which applies to a single iteration (LLM call plus execution of requested tool calls):
...
> [!IMPORTANT]
>
> If two rules have the same intent but different targets, the one with higher priority is applied.
...
```python
ConditionalRequirement(tool_a, min_invocations=1)
```
