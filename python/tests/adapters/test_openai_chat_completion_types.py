...
def test_arguments_accept_a_json_string() -> None:
    ...
    decorator."""
    assert ChatToolFunctionDefinition.model_validate({"name": "f", "arguments": '{"a": 1}'}).arguments == {"a": 1}
...
def test_invalid_json_arguments_raise() -> None:
    with pytest.raises(ValueError):
        ChatToolFunctionDefinition.model_validate({"name": "f", "arguments": "not json at all"})
