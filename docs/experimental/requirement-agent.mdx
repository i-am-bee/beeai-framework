...
- Select appropriate tools intelligently based on context and workflow requirements
...
In RequirementAgent, **all capabilities (including data retrieval, web search, reasoning patterns, and final answers) are implemented as tools** to ensure structured, reliable execution. Each call returns exactly one `Rule` with:

| Attribute | Purpose | Value |
|-----------|---------|-------|
| `target` | Which tool the rule applies to | Tool class or instance |
| `allowed` | Whether the tool can be used | Boolean |
| `hidden` | Whether the tool definition is visible to agent | Boolean |
| `prevent_stop` | Whether rule blocks termination | Boolean |
| `forced` | Whether tool must be invoked | Boolean |
...
ConditionalRequirement(WeatherTool, consecutive_allowed=False)
```
