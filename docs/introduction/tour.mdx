...
<Tip>
Caching works for LLM calls and individual tools, not for entire agents, because agent inputs are usually unique.
</Tip>

**1. Enable LLM Caching**

You need to explicitly configure a cache for your LLM:
...
async def main():
    ...
    # Example usage
    response1 = await llm.chat("Hello! How are you today?")
    response2 = await llm.chat("Hello! How are you today?")  # Should hit the cache

    print("First response:", response1.text)
    print("Second response (cached):", response2.text)
...
Tools that make API calls can also be cached to avoid repeated requests:
...
**3. Using Cached LLMs & Tools in an Agent**
...
agent = RequirementAgent(
    llm=llm,  # LLM with caching enabled
    memory=UnconstrainedMemory(),
    tools=[weather_tool],
...
print(response1.last_message.text[:100] + "...")
print(response2.last_message.text[:100] + "...")
```
