...
<Tip>
  You can also use other LLM providers like OpenAI, Anthropic, or watsonx - see
  [Backend](/modules/backend) to learn more about supported providers.
</Tip>
...
<Info>Related Module: [Tools](https://framework.beeai.dev/modules/tools)</Info>
...
const response = await agent.run({
  prompt:
    "What's the current weather in New York and tell me about the history of the city?",
});
console.log(response.result.text);
```
...
<Tip>
  ...
  BeeAI's suggested agent implementation for reliability and control over agent
  behavior.
</Tip>
...
<Info>
  Related Modules: [Emitter](/modules/middleware),
  [Events](/modules/middleware), [Observability](/modules/observability).
</Info>
...
<Tip>
  `Middleware` can be attached per-run (affects only that run) or at agent
  construction (applies to all runs, no need to attach each time).
</Tip>
...
<Tip>
  Listening for all events can be noisy. Filter to the events you are interested
  in capturing.
</Tip>
...
<Info>**Relevant Module**: [Logger](/modules/logger)</Info>
...
<Note>
  To run this step: `pip install openinference-instrumentation-beeai
...
<Tip>
  Learn more about Requirements and see examples in
  [documentation](/modules/agents/requirement-agent)
</Tip>
...
<Info>Relevant Module: [RAG](/modules/rag)</Info>
...
<Tip>
  For complex, multi-step processes, see the [Workflows
...
<Info>Relevant Module: [Cache](/modules/cache)</Info>
...
<Note>
  Caching the entire agent isn’t practical—every agent run is usually unique.
  Instead, focus on caching the components inside your agent.
</Note>
...
<Info>
  Relevant Module: [Serve](/modules/serve), [MCP](/integrations/mcp),
  [A2A](/integrations/a2a), [IBM watsonX
  Orchestrate](/integrations/watsonx-orchestrate)
</Info>
