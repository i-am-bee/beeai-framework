...
Pass `timeout` (in seconds) to `ToolRunOptions` to limit how long a single tool call is allowed to run. When the deadline is exceeded, a recoverable `ToolError` is raised (the agent catches it, feeds it back to the model, and continues).

```py Python
result = await tool.run(input, options=ToolRunOptions(timeout=10.0))
```

The timeout is **per-attempt**: each retry gets a fresh deadline, so `timeout=5` with `max_retries=3` bounds each individual attempt at 5 s, not the total run.

<Note>
The timeout can only interrupt the tool at `async` suspension points. Blocking calls such as `time.sleep()` or `requests.get()` will not be interrupted, when not using `await`.
</Note>
...
<Note>
  For an authenticated hosted MCP workflow, see the [Baizhi research example](https://github.com/i-am-bee/beeai-framework/blob/main/python/examples/tools/mcp_baizhi_research.py). Set `BAIZHI_API_KEY` and, optionally, `BAIZHI_MCP_ENDPOINT` before running it. The example keeps the HTTP and MCP sessions open for the agent run, discovers paginated tools, allows only the three research tools, and limits each to one invocation. Tool arguments are sent to the hosted service and may incur service or model costs.
</Note>
...
If your tool is providing data to the agent, try to ensure that the data is relevant and free of extraneous metadata. Preprocessing data to improve relevance and minimize unnecessary data conserves agent memory, improving overall performance.
