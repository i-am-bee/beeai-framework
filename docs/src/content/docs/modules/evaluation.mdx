...
|-----------------|-----------|-------------------|
| `question`, `answer` | `question`, `answer` | Copied as-is |
| `supporting_facts` (`title` + `sent_id` pointers) | `relevant_sentences` | Each pointer is resolved against `context` and the actual sentence text is copied in, instead of kept as an index |
| `supporting_facts.title` (deduplicated) | `supporting_titles` | The distinct article titles the supporting facts point to |
| *(no HotpotQA equivalent)* | `wiki_times` | Count of distinct `supporting_titles`; how many Wikipedia lookups the agent is expected to make |
...
| `relevant_sentences`  | Relevant facts the agent should reference               |
| `supporting_titles`   | Source titles for the supporting context                |
| `wiki_times`          | Expected number of Wikipedia tool calls                 |
