...
`RequirementAgent`, `ToolCallingAgent`, and `ReActAgent` factories. These options
apply to every request served by that registration, including cloned agents.
Omitted fields and fields set to `None` retain the agent's own defaults; explicit
integer values (including `0`) are passed through with the agent's existing semantics.
The configuration is copied during registration, so changing the original object
afterwards does not change the server's behavior. Non-empty execution configuration
is rejected for other runnable types. Custom factories retain their existing signature
and are responsible for consuming execution metadata when applicable.
...
<CodeGroup>
	...
	{/* <!-- embedme python/examples/serve/agent_stack_rag.py --> */}
