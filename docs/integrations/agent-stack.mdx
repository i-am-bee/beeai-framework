...
<Heading level={2} id="client">Consuming from the stack (client)</Heading>
...
<Heading level={2} id="server">Exposing to the stack (server)</Heading>

The `AgentStackServer` class exposes an agent or any other runnable (tool/chat model, ...) to the **Agent Stack**.
It gets automatically registered to the stack and allows you to access and use the agents directly in the stack.
...
<Tip>
	You can let the Agent Stack to do LLM inference by using the `AgentStackChatModel(...)` class.
</Tip>
...
It does so by leveraging the [Citation Extension](https://docs.beeai.dev/build-agents/citations), which is managed by the  `CitationMiddleware` class.
