...
[BeeAI Platform](https://beeai.dev/) is an open platform to help you discover, run, and compose AI agents from any framework.
This tutorial demonstrates how to consume agents from the BeeAI Platform and expose agents built in BeeAI Framework to the BeeAI Platform.

**Prerequisites**

- **[BeeAI Platform](https://beeai.dev/)** installed and running locally
- **BeeAI Framework** installed with `pip install beeai-framework[beeai-platform]`
...
The `BeeAIPlatformServer` class exposes an agent or any other runnable (tool/chat model, ...) to the **BeeAI Platform**.
...
<Note>
	BeeAI Platform supports only one entry per server. To register more, you need to spawn more servers.
</Note>

<Tip>
	You can use the platform to do LLM inference by using the `BeeAIPlatformChatModel(...)` class.
</Tip>
...
The server's behavior can be influenced via attributes listed in `BeeAIPlatformServerConfig` class (host, port, self registration, ...).
...
```python
from beeai_framework.adapters.beeai_platform.serve.server import BeeAIPlatformServer, BeeAIPlatformServerConfig
from beeai_framework.serve.utils import LRUMemoryManager

memory_manager = LRUMemoryManager(maxsize=64) # keeps max 64 conversations
config = BeeAIPlatformServerConfig(host="127.0.0.1", port=9999, run_limit=3600, limit_concurrency=10)
server = BeeAIPlatformServer(config=config, memory_manager=memory_manager)
```
...
agent = RequirementAgent(llm=BeeAIPlatformChatModel())
...
The BeeAI Platform has a concept of [extensions](https://docs.beeai.dev/concepts/extensions) that enable access to external services and UI components via dependency injection.
...
<Note>The current entries are listed in the `BaseBeeAIPlatformExtensions` class.</Note>
...
<CodeGroup>
	...
	{/* <!-- embedme python/examples/serve/beeai_platform_custom.py --> */}
...
<Tip>The `BeeAIPlatformContext.get()` can be called from anywhere if the code is running inside the given context.</Tip>
