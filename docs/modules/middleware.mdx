...
Middleware in the BeeAI Framework is the "connective tissue" that allows you to observe, intercept, and modify an agent's behavior without altering its core logic.

<Note>
Note: **Middleware** here refers to the classic software design pattern, it is the code that runs between the agent's decision steps. This is distinct from the newer industry term "Agentic Middleware," which may refer to entire agent orchestration platforms.
</Note>

The BeeAI Framework core is designed to be extensible. Middleware leverages the underlying system of **Events** and **Run Contexts** to hook into the agent's lifecycle. Whether you need to log a thought process, sanitize an input, or halt a dangerous action, middleware provides the interface to inject that logic.
...
The fastest way to see what's happening in your code (who is calling what) is by using the `GlobalTrajectoryMiddleware`. It captures all events, even deeply-nested, and prints them to the console, visualizing nested calls with indentation.
...
Middleware for handling streaming tool calls in a ChatModel. It observes [Chat Model](/modules/backend#chat-model) stream updates and parses tool calls on demand so that they can be consumed immediately. It works even without streaming enabled (emitting the `update` event at the very end).
...
The emitter instance has the following **methods**:
...
The following sections explain how to use emitters to capture, observe, and modify events.
