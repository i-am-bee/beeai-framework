...
The backend system manages your connection to different language model providers. The BeeAI Framework supports multiple LLM providers through a unified interface. Learn more about available backends in our [backend documentation](./backend).
...
# Using Anthropic
llm = ChatModel.from_name("anthropic:claude-3-sonnet")
...
#### Setting Execution Settings with `AgentExecutionConfig`
...
execution_config = AgentExecutionConfig(
    total_max_retries=10,        # Maximum total retries across all steps
    max_retries_per_step=3,      # Maximum retries for each individual step
...
<Tip>
The default is zero retries and no iteration limit. For complex tasks, increasing max_iterations is recommended to allow thorough reasoning.
</Tip>

####  Guiding Agent Run Behavior
When executing an agent, you can provide additional options to guide its behavior:
...
- Execution settings can also be set directly in the run call
...
<Tip>
The ReAct Agent is available in both python and type script, but no longer actively supported in Python.
</Tip>
...
<Tip>
The Tool Calling Agent is available in both python and type script, but no longer actively supported in Python.
</Tip>
...
<Warning>
<strong>Upcoming change:</strong><br />
Workflows are under construction to support more dynamic multi-agent patterns. If you'd like to participate in shaping the vision, contribute to this [V2 Workflow Proposal](https://github.com/i-am-bee/beeai-framework/discussions/1005) github discussion.
</Warning>
