...
You can attach files (e.g. PDFs) to a `UserMessage` using the `MessageFileContent` part. Provide either a remote `file_id`/URL or an inline base64 data URI (`file_data`). Optionally specify a MIME `format`.

```py Python
from beeai_framework.backend import UserMessage, MessageFileContent

# Using a remote / previously uploaded file URL or id
msg_with_file_id = UserMessage([
  ...
  # You can mix text with file parts
...
These content parts are passed through to supported providers via the LiteLLM adapter using the schema:

```json
{
  "type": "file",
  "file": { "file_id": "..." } // or { "file_data": "data:application/pdf;base64,..." }
}
```
